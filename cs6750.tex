...
The need-finding approach covered in this section connects the user's goals of altering a data feed to the sub-tasks of viewing the existing data, planning the transformation, and confirming the outcome. In the scenarios presented above, the workaround descriptions can be used to inform the interface designers about system's limitations and constraints. The approach of need-finding via analysis of hacks and workarounds is susceptible to \textit{observational selection bias} as we tend notice workarounds when they impact us directly. To avoid this bias, we pair up this approach with another method that seeks to understand pain points of a larger group of users, preferably with diverse roles.
